...
- **Known latent bug:** `useIntroSession.js` has no `genFailed` handling, so a failed intro render silently drops the user to the script step. Unreachable today because `intro` is absent from `Creator.jsx`'s `ENABLED_MODES`. Fix it when the intro pipeline is re-enabled.
