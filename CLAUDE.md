...
- Resume is driven only by `?session=`. `getSession` on the backend runs `reconcileIfStuck` on **every read**, so the frontend's own 5s poll is what triggers the 15-min stuck-generation watchdog. A user who closes the tab is covered by `sweepStuckGenerations` (5 min) and `sweepStuckJobs` (5 min, the generic job slot).
- **`startGeneration` must link the Video row before anything fallible runs.** Its catch and `reconcileIfStuck` both mark failure on that row; with no row there is no marker, `isGenerationFailed()` returns false, and the generating screen polls forever. `claimGenerationLock` returns a claim token and clears the previous attempt's FAILED marker as it flips to GENERATING, so `FAILED` and `GENERATING` can never coexist - which is why `isGenerationFailed()` (`src/lib/progressTasks.js`) deliberately does **not** also test `stage !== "GENERATING"`. Pass the token back to `releaseGenerationLock` so an early abort restores the error.
- **Auto-approve is per-session, not live.** The backend snapshots the toggles onto `WizardSession.autoApprove` at creation. Read it via **`useResolvedAutoApprove(session)`** and feed that one object into **both** the gates (`autoActive`/`manualGate`) and the journey checklist (`buildImageTasks({ prefs })`). Reading `useAuth().autoApprove` directly in a creator is a bug: a toggle flipped mid-run then changes how the run resumes, and the checklist stops matching the gates.
- The auto-advance effect fires on resume by design, so `run("generate", ...)` is gated on `generationAttempts === 0` - `failedGate`/`steppedBack`/`autoDisabled` are component state and are lost on reload.
- Every backend stage needs an entry in `getStageToStep()` **and** in the `status='in-progress'` allowlist in `video.service.js` (`listSessions`). `RESTYLING` was missing from both, so those sessions vanished from MyVideosPage and resumed onto step 0.
- Resume mode: use `resumeModeFor()` (`src/lib/pipelineMode.js`). `prompt` and `image` share `ImagePipelineCreator` but are distinct `?mode=` values.
- The Brand Intro pipeline is **live again**: `intro` is in `RESUMABLE_MODES`/`ENABLED_MODES`, has its own `MODE_TABS` entry in `Creator.jsx`, and a `ModeChooser` card. `useIntroSession.js`'s `genFailed` handling (the old latent bug) is in place and mirrors `useSession.js` - a failed intro render pins to `GENERATING_STEP` with the failure screen instead of dropping to the script step.
- **Generation logging:** `logEvent`/`logObserve`/`logFailure` (`src/lib/genLog.js`) write to console + Slack telemetry + `merge-api/logs/generation.log` (via `POST /api/telemetry/gen-log`, which the server also writes to). Log **transitions only** - the endpoint allows 60 posts / 5 min, which an undeduped 5s poll saturates. Raw logs never render in the UI; the user-facing surface is the reason string on the failed checklist row.
- **The backend owns the pre-script pipeline; the tab only observes.** `pipelineRunner.service.js` (`advanceSession`) walks `IMAGES_UPLOADED -> analyze -> RESTYLING -> IMAGES_ANALYZED -> outline -> OUTLINE_GENERATED -> approveOutline` from the persisted stage + `autoApprove`, so closing the tab can no longer abandon a run. It is re-entered from three places: `POST /:id/advance`, each job's `onDone` (added to `startSessionJob`), and `sweepStalledSessions`. Two invariants: **(1)** `onDone` fires on **DONE only** and the sweep skips `jobStatus='FAILED'` for a pipeline job type - re-dispatching a failed step re-claims the slot, fails, and re-enters, which is a hot retry loop against OpenAI; **(2)** the chain **stops at `SCRIPT_GENERATED`** - starting the render needs the PiAPI preflight, the concurrency cap, and `backgroundMusic`, which is *never persisted on WizardSession*, so driving it server-side would force music on for users who turned it off. Restyle is polled inline, never held in the job slot (it would block the outline and expose a foreign `jobType` to a client polling the slot). The sweep only rescues sessions stalled **>10 min and <24 h**: reviving an older draft charges credits and mails a script nobody is waiting for.
- **`scriptData == null` does NOT mean the script phase is idle.** A healthy run sits at step 1 with a null script while the backend writes it. The merged-run overlay may cover step 1 only while work is provably in flight (`loading`, `jobStatus === 'RUNNING'`, or stage `IMAGES_UPLOADED`/`RESTYLING`); otherwise `atManualReview` must uncover ScriptStep's Generate/Retry button, or the overlay hides the only control that can advance the session. Confirm the idle state **holds** for `SCRIPT_STUCK_GRACE_MS` first - the backend's job-to-job handoff briefly reads as stuck.
...
- Bottom action bar is **context-sensitive**: no selection → Assets / Audio / Voice / Zoom±; clip selected → Split / Speed / Delete (+ **Regenerate** for video clips, + **Volume** & **Narration** for audio clips). A slim **"Done editing clip"** bar above the action bar deselects. Assets/Speed/Volume/Narration open bottom sheets or modals; mobile audio-adds auto-place on the timeline (no drag).
- **Audio rows = three, grouped by source kind (not stored trackIndex):** **Narration** (item has `sectionId`, or asset `sourceType==="TTS"`), **Audio** (uploads / anything else), **Music** (`sourceType==="AI_MUSIC"`). The `audioKind()` helper in `TimelineCanvas.jsx` is the source of truth; `TimelineEditor`'s `audioTrackIndex()` + the Assets sheet, and `AssetPanel`, mirror it. Display groups by kind so existing timelines map correctly regardless of the trackIndex stored by the backend (`timelineMusic.js` places AI_MUSIC at index 1). On add, trackIndex is set to the kind index (narration 0 / audio 1 / music 2).
- **Per-clip volume + multi-track are backend-ready** - the ffmpeg export mixes ALL audio items with per-item `volume` (clamped 0-2.0 in `timeline.service.js` `updateItem`); the editor only needed UI. The Volume sheet commits on slider release; `VideoPreview` clamps the HTML element's `.volume` to 0-1 (export still applies >1).
