    ...
    <div className="w-full h-full flex flex-col lg:flex-row">
      ...
      <div className="flex-1 flex flex-col p-6 border-r border-gray-100 overflow-hidden">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">{introScript?.businessName || "Brand Intro"} Script</h2>
            <p className="text-sm text-gray-600">{vignettes.length} montage beats • ~8s intro</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-5 pr-1">
          {/* Business name */}
          <div>
            <label className="text-xs font-bold uppercase tracking-widest text-stone-500 mb-1 block">Brand name</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-bold uppercase tracking-widest text-stone-500 flex items-center gap-1"><Film className="w-3.5 h-3.5" /> Montage beats</label>
              <button onClick={addVignette} className="text-xs font-bold text-[#F97066] inline-flex items-center gap-1"><Plus className="w-3.5 h-3.5" /> Add</button>
            </div>
            <div className="space-y-2">
              {vignettes.map((v, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2">
                  <span className="w-6 h-6 shrink-0 rounded-full bg-orange-100 text-[#F97066] text-xs font-bold flex items-center justify-center">{i + 1}</span>
                  <Input value={v} onChange={(e) => setVignette(i, e.target.value)} className="flex-1" />
                  <button onClick={() => removeVignette(i)} className="text-stone-400 hover:text-red-500"><X className="w-4 h-4" /></button>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          {/* Motion prompt */}
          <div>
            <label className="text-xs font-bold uppercase tracking-widest text-stone-500 mb-1 block">Motion prompt</label>
            <Textarea rows={4} value={introScript?.motionPrompt || ""} onChange={(e) => updateScriptField("motionPrompt", e.target.value)} />
          </div>

          {/* Narration */}
          <div>
            <label className="text-xs font-bold uppercase tracking-widest text-stone-500 mb-1 flex items-center gap-1"><Mic className="w-3.5 h-3.5" /> Narration <span className="normal-case font-medium tracking-normal text-stone-400">(optional — leave empty for music only)</span></label>
          ...
          {hasNarration && (
            <div>
              <label className="text-xs font-bold uppercase tracking-widest text-stone-500 mb-2 block">Narration voice</label>
          ...
          {/* Music prompt */}
          <div>
            <label className="text-xs font-bold uppercase tracking-widest text-stone-500 mb-1 flex items-center gap-1"><Music className="w-3.5 h-3.5" /> Background music</label>
      ...
      <div className="w-full lg:w-80 flex flex-col bg-gray-50 p-6">
        ...
        <p className="text-sm text-gray-600 mb-4">Describe a change and AI will revise the whole script.</p>
        <Textarea
          value={editRequest}
        ...
        <div className="mt-6 p-3 bg-purple-100 rounded-lg border border-purple-200">
          <p className="text-xs text-purple-800"><strong>Tip:</strong> edit any field directly, then Approve &amp; Generate.</p>
        </div>
      </div>
    </div>
  );
